	...
	<div class="col-sm-12">
		<div ng-controller="ActiveFileController">
			<h3 style="margin-top: 0px;">
				<button class="btn btn-default btn-xs pull-right" ng-click="closeActiveFile()">Hide</button>
			...
			<p class="clearfix">
				...
				<a href="{{ route('admin.ajax.files.index') }}/@{{ activeFile.id }}/download" class="btn btn-default btn-xs">Download</a>
				<button ng-click="createFileLink(activeFile)" class="btn btn-primary btn-xs">Create Download Link</button>
				<button ng-click="changeFilePath(activeFile)" class="btn btn-warning btn-xs">Change Path</button>
				<button ng-click="deleteFile(activeFile)" class="btn btn-danger btn-xs">Delete</button>
			</p>
			...
			<div ng-show="fileLinksOfFile(activeFile).length > 0" style="margin-top: 40px;">
				...
				<div ng-repeat="fileLink in fileLinksOfFile(activeFile)">
					...
					<div class="pull-right">
						<button class="btn btn-danger btn-xs" ng-click="deleteFileLink(fileLink)">Delete</button>
						<button class="btn btn-default btn-xs" ng-click="editFileLink(fileLink)">Edit</button>
					</div>
					<p class="link-name">
